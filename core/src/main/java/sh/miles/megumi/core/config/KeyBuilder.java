...
public class KeyBuilder {
    ...
    public void parseLine(String line) {
        ...
        // If the path doesn't exist in the config it keeps removing last key in
        // keyBuilder.
        ...
        // If currentSplitLine[0] is 'key2' and keyBuilder contains 'key1' the result
        // will be 'key1.' if '.' is the separator
        if (builder.length() > 0)
            builder.append(separator);

        // Appends the current key to keyBuilder
        // If keyBuilder is 'key1.' and currentSplitLine[0] is 'key2' the resulting
        // keyBuilder will be 'key1.key2' if separator is '.'
        builder.append(key);
    }
    ...
    // Checks to see if the full key path represented by this instance is a sub-key
    ...
    public void removeLastKey() {
        ...
        // Must be enclosed in brackets in case a regex special character is the
        // separator
        String[] split = keyString.split("[" + separator + "]");
        // Makes sure begin index isn't < 0 (error). Occurs when there is only one key
        // in the path
